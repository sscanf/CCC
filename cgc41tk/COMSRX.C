...
extern int ErrorFichero;

extern unsigned int  comndAct;
extern ptrCmndInfo listExec;
...
//  sio_enableTx(PORT7+3);
  txData=sio_oqueue(PORT10);
  freeSpace=sio_ofree(PORT10);
  if (txData>=0 && freeSpace>=7)
  {
//		delay(5);
		sio_putch(PORT10,0xec);
//		delay(5);
		sio_putch(PORT10,0xa1);
//		delay(5);
		switch (comanda)
		{
		 case CONFIG:
...
//		 chOut=chOut&0x3f; /* 11 chOut <10 comanda de config> */
//		 printf("CONFIG ChOut %x\n",chOut);
//		 printf("ChIn %x\n",chIn);

		 sio_putch(PORT10,chOut);
		 sio_putch(PORT10,chIn);
		 break;
		 }
		 case INI_CONFIG:
		 {
...
void interrupt IntPort2()
{
  ...
  unsigned FiRx=0,numBloques=0,length=0,trobat=0,i;
  static int ch;
  ...
  if (ch>=0)
  {
	  ...
	  if (FiRx==1) // FI RECEPCIO
	  {
		 ...
		 if ((comanda!=RES && canalASI!=-1) ||  comanda==ERRHOST || comanda==NIT)
		 {
			...
			if (comanda==NIT)
			  memccpy(bufRxData,bufRxHeader2,0,MAX_BUFFER_VAX+1);
			else
			  if (comanda==ASI)
			  {
			...
			err=0;
			posaTasca(0,bufRxData,comanda,0,0,atoi(bufRxHeader2),NOTRUN);
		 }
		 else
		 {
			...
			{
			  bufRxData[9]='\0';
			  posaTasca(0,bufRxData,ERR,comanda,0,atoi(bufRxHeader2),NOTRUN);
...
{
 char dest[20]="",dest2[20],expedient[20],numServei[20];
...
// sio_enableTx(PORT7+3);
 ...
 txData=sio_oqueue(PORT9);
 freeSpace=sio_ofree(PORT9);
...
//	 sio_write(PORT9,dest,strlen(dest));
//	 printf(dest);
	 ...
	 switch(comanda)
	 {
	  ...
	  case LLI:
			  ...
			  if (numbytes!=0)
			  {
				strcat(dest,missatge);
				sio_write(PORT9,dest,strlen(dest)+numbytes);
			  }
			  else
				sio_write(PORT9,dest,strlen(dest));
	  ...
	  case ERR:
				 ...
				 {
				  case ASI:strcat(dest,"ASI");break;
				  case MOD:strcat(dest,"MOD");break;
				  case ANU:strcat(dest,"ANU");break;
				  default:strcpy(dest,"");
				 }
//				 sio_write(PORT9,dest,strlen(dest));
...
//				 sio_write(PORT9,dest2,strlen(dest2));
				 sio_write(PORT9,dest,strlen (dest)+numbytes);
