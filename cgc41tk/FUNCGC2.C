...
// if (operador>=2) return;
 ...
 do{
  ...
  if (!DebugMode)
  {
	  window (20,18,38,24);
	  gotoxy (x,y);
	  cprintf ("%x %x %x %x %s\r\n",operador,comanda,numProc,numbytes,missatge);
	  y=wherey();
	  x=wherex();
	  window (1,1,80,25);
  }
...
//		cprintf("connecta\r\n");
	  ...
	  if (listExec[0].comnd!=SQLON) // MSSERVEIS, MSEMER o VEU
	  {
		...
		{
			gotoxy (25,3);
			if (listExec[0].comnd==MSSERVEIS ) printf ("VEU");
			if (listExec[0].comnd==MSEMER ) printf ("SOS");

			gotoxy (32,3);
			printf ("%s ",listExec[0].info);
		}
...
//   if (listExec[0].proces==MSEMER)
//       strcat (listExec[0].info," *");
	  ...
	  // SqlOn
		...
		//       per l'operador s¢n el 1,2,3,4,...
		 ...
		 strcat (dest,itoa((listExec[0].origen+1),dest2,10));
...
//
 ...
 for(i=0;i<lastExec && i<MAX_NUM_EXEC;i++)
 {
  ...
  strcpy(listExec[i].info,listExec[i+1].info);
 }
 ...
 memset(listExec[lastExec].info,'\0',MAX_BLOQUE);
...
int tascaEspera(unsigned int entrada,unsigned int sortida,unsigned int flag)
{
 ...
 for(;listExec[0].estat!=FIESPERA && time<TIMEOUT*10;time++);
 if (time==TIMEOUT*10) return (-1);
 else
 return (0);
...
// Envia la comanda a MasterClk per llegir estat actual de la connexi¢.
	 ...
	 while (tascaEspera(canal,0,0)<0) Tx2MasterClk(RXESTAT,canal,0,0);
...
// encarregat de modificar indicAct canalAct
 ...
 else
 {
	...
	if (canal>-1)
	{
		...
		listIndics[posicio].flota=1;
		listIndics[posicio].indicatiu=indicatiu;
		listIndics[posicio].canal=canal;
		listIndics[posicio].subto=subto;
...
// Funci¢ que gestiona PTT.
  unsigned int i,ctrlPTT=0,flgCanal=0;
  char dest[MAX_LONG_INFO]="";
  char *tempo;
  int canalTx=-1;
...
//             printf("PTT Canal On");
					...
					while (tascaEspera(listExec[0].origen+0x0a,0,0)<0)
						Tx2MasterClk(RXESTAT,listExec[0].origen+0x0a,0,0);
					...
					while (tascaEspera(canalTx,0,0)<0)
						Tx2MasterClk(RXESTAT,canalTx,0,0);
...
// est… en marxa. Es considera que si respon ‚s perquŠ tot va b‚.
 ...
 Tx2Oper(listExec[0].origen,listExec[0].comnd,obrir,0,"",0);
 sleep(3);
...
//    printf("PTT Codis On\n");
		...
		for(TimeOut=0;((listCanals[canalAct].estat==ESPERA )&& TimeOut<3777340);TimeOut++);
...
void gestioErr(void)
{
 ...
 // S'avisa a l'operador que no s'ha pogut comunicar amb el m•bil.
	retError(listExec[0].proces,6);
 else
	{
//  posicio=posicioIndicatiu(listExec[0].origen);
//  if (posicio==-1 || listIndics[posicio].numReTx==0 || lastExec>=MAX_NUM_EXEC-1)
	 // Si l'indicatiu al que s'enviava el missatge no existeix o
	 // el num de reTx ha arribat al m…xim, error al VAX.
//  {
		Tx2VAX(listExec[0].comnd,listExec[0].origen,strlen(listExec[0].info),
				 listExec[0].info);
//    if (posicio!=-1)  listIndics[posicio].numReTx=NUM_RETX;
//  }
//  else
//   if (posicio!=-1 && listIndics[posicio].numReTx>0)
	  // Cal una reTx del fitxer
//   {
//     printf("ReTx fitxer %d\n",listIndics[posicio].numReTx);
//     listIndics[posicio].numReTx--;
		 // ASI,ANU,MOD
//     listExec[lastExec].comnd=listExec[0].proces;
		 // indicatiu
//     listExec[lastExec].origen=listExec[0].origen;
//     listExec[lastExec].estat=NOTRUN;
//     listExec[lastExec].proces=0;
//     if (listExec[0].comnd==MOD || listExec[0].comnd==ANU)
//      strcpy(listExec[lastExec].info,listIndics[posicio].fitxer);
//     else strcpy(listExec[lastExec].info,"");
//     actLastExec();
//   }
	}
...
void retardaTasca(void)
{
 ...
 // retardem l'actual
  ...
  listExec[lastExec].origen=listExec[0].origen;
  strcpy(listExec[lastExec].info,listExec[0].info);
  actLastExec();
 tascaRetard=OK;
...
//     strcpy(listIndics[posicio].fitxer,listExec[0].info);
		 listExec[0].info=tractaFitxer2(listExec[0].info);
		 strcpy(listCanals[canal].info,listExec[0].info);
...
{  // demana subto
	...
	// per guardar-lo en listIndics quan arribi missatge
	 ...
	 int x;
	 unsigned int canal,subto=0,sqlOn=1,AntSubto=0,time2,flg=0;
...
//		 printf("SQLON subto %d ch %d   time %d\n",subto,canal,time);
			 if (((subto==1 && canal==3) || (subto==13 && canal==0) || (subto==15 && canal==1)
  ...
  canal=listExec[0].origen;
	  for(i=0;i<ACT_OPER;i++)
	  {
		...
		if (permis==3 || permis==1) // En Rx o Tx/Rx
		{
		 ...
		 listExec[0].estat=ESPERA;
		 Tx2MasterClk(RXESTAT,i+0x0a,0,0);
		 while (tascaEspera(i+0x0a,0,0)<0);
//			 Tx2MasterClk(RXESTAT,i+0x0a,0,0);
		 ...
		 if (listExec[0].estat==FIESPERA)
		 {
		  ...
		  noConnexio=(listExec[0].proces&0x40)>>6;
...
int canalActiu(unsigned int canal)
{
	 ...
	 if (sqlOn==0)
	 {
		 ...
		 {
//		 printf("ch %d actiu",canal);
			return OK;
		 }
		 else
		 {
//		 printf("ch %d NO actiu",canal);
		  return NOTOK;
		 }
	 }
	 else
...
void gestioCarrega(void)
{
	 ...
	 if (listCmnd[comndAct].comnd!=RES)
	 {
	  ...
	  strcpy(listExec[lastExec].info,listCmnd[comndAct].info);
	  ...
	  memset(listCmnd[comndAct].info,'\0',MAX_BLOQUE);
	  actLastExec();
	 }
 ...
 for(i=0;i<lastExec && i<MAX_NUM_EXEC;i++)
 {
  if (listExec[i].comnd==SQLON && listExec[i].origen==canal)
  {
	...
	for(j=i;j<lastExec && j<MAX_NUM_EXEC;j++)
	{
	 ...
	 strcpy(listExec[j].info,listExec[j+1].info);
	}
	if (lastExec>0 && lastExec<(MAX_NUM_EXEC-1)) lastExec--;
...
{
/*
 unsigned int indicatiu,flota,validOrg=0;
 unsigned int validInfo=0,validIndic=0,validFlota=0,comanda=RES,j=0,trobat=0,i=0;
