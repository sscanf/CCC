...
unsigned char bufConfig[ACT_CH][ACT_OPER];
...
extern ptrCmndInfo listCanals,listExec;
...
int IniSio(void)
{
	...
	for (n=0;n<11;n++)
	{
	  ...
	  sio_cnt_irq   (PORT0 ,IntPort0 ,1); // C¢nsola Operador coches taller

	  sio_cnt_irq   (PORT1 ,IntPort1 ,1); // C¢nsola Operador gruas

	  sio_cnt_irq   (PORT2 ,IntPort8 ,1); // C¢nsola T‚cnico

	  sio_cnt_irq   (PORT3 ,IntPort9 ,1); // C¢nsola Trunking

	  sio_cnt_irq   (PORT4 ,IntPort3 ,1); // Canal 1 st 1
	  sio_modem_irq (PORT4 ,IntFitxer3);

	  sio_cnt_irq   (PORT5 ,IntPort4 ,1); // Canal 2 st 2
	  sio_modem_irq (PORT5 ,IntFitxer4);

	  sio_cnt_irq   (PORT6 ,IntPort5 ,1); // Canal 7 st 3
	  sio_modem_irq (PORT6 ,IntFitxer5);

	  sio_cnt_irq   (PORT7 ,IntPort6 ,1); // Canal G st 4
	  sio_modem_irq (PORT7 ,IntFitxer6);

	  sio_cnt_irq   (PORT8 ,IntPort10,1); // Puerto trunking st 5

	  sio_cnt_irq   (PORT9 ,IntPort2 ,1); // Vax

	  sio_cnt_irq   (PORT10,IntPort7 ,1); // Master Clock
...
void colocaCmnd2(unsigned int posRefAct,unsigned char *header,char *info,unsigned int origen)
{
	...
	if (comanda==MSSERVEIS || comanda==MSEMER || comanda==SEND_PROG || comanda == CLEARED || comanda == TRUC_SOCI)
	{
		itoa(tempo,info,10);
	...
	//  En .origen l'indicatiu.
	 ...
	 if (listCanals[origen].comnd==PERVEU || listCanals[origen].comnd==TRUCSOCI)
	 {
		 itoa(tempo,info,10);
		 strcpy(info,ompleZeros(info,3));
		 tempo=atoi (listCanals[origen].info);
		 origen=origen2; // .origen
	 }

	}

	if (comanda==SEND_PROG || comanda==CLEARED)
		tempo=atoi (listCanals[origen].info);
...
/* El missatge que s'espera rebre ‚s: 0xec 0xa1 numProc comanda numbytes <info de numbytes de llarg…ria>*/
	...
	if (carac>=0)
	{
	  ...
	  if (FiRx==1) // FI RECEPCIO
	  {
		 gestRxIntOpers(PORT3,bufRxHeader,bufRxData);
  ...
  if (freeSpace>=numbytes+HEADER+2+3)
  {
	  if (sio_lctrl(canal,C_RTS)==0)
	  {
		...
		if (header==1)
		{
		 ...
		 //ahora    8
		 if (canal==8) flota=29; //En el canal trunking la flota siempre es 29
		 ...
		 if (canal==8)
		 {
			if (indicatiu<MAX_TABLA)
				indicatiu = TablaPredefs[indicatiu-1];
		  else
				indicatiu+=NumBase;
		 }
		...
		if (header==0 || comanda==MOD || comanda==PROGINDIC || comanda==ASI || comanda==PERVEU || comanda == TRUCSOCI)
		{
		  // info del bloc en cas de transmissi¢  de fitxer i nou prefixe e indicatiu
		  if (comanda==MOD || comanda==ASI)
		  {
			 lng=strlen (missatge);
			 if (sio_ofree(canal)>=strlen (missatge))
				sio_write(canal,missatge,strlen(missatge));
		  }

		  else
		  if (comanda==PROGINDIC)
		  { // PROGINDIC
		  ...
		  if (comanda==PERVEU || comanda==TRUCSOCI)
		  {
			 dest[0]=listExec[0].info[3];
			 dest[1]=listExec[0].info[4];
			 dest[2]=listExec[0].info[5];
			 ...
			 indicatiu=atoi(dest);
			 posicio=posicioIndicatiu(indicatiu);
			 listCanals[posicio].comnd=comanda;
...
/* El missatge que s'espera rebre ‚s: 0xec 0xa1 flota indicatiu<2 bytes> comanda numbytes <info de numbytes de llarg…ria>*/
	...
	static unsigned int numbytes=0;
	static unsigned long int i=0;
	static char AntByte=0;
	char far *pp;
	static int fram=0;



	carac=sio_getch(PORT8); // PORT8 ahora
//	cprintf ("%x ",carac);  //Ahora

	if (carac==0xa1 && AntByte==(char)0xec) ptrRx=0;
...
//		 cprintf ("\n\r");  //Ahora
		 gestioRxIntRadio(4,bufRxHeader,bufRxData);
...
void interrupt IntPort7(void)
{
  ...
  static int ptrRx=-2;
	...
	ch=sio_getch(PORT10); //PORT10 hora
	 if (ch>=0)
	 {
	  switch(ptrRx)
	  {
 ...
 if (status2==0) //  CTS OFF , canal lliure
 {
  ...
  // No hi ha problemes encara que ho enviem ja que ST no s'ho mira.
	...
	for(i=0;i<MAX_BLOQUE && listCanals[0].info[i]!='\0';i++)
	 if (i<MAX_BLOQUE) dest[i]=listCanals[0].info[i];;
		...
		// Si el indicativo ya existe, lo quita para despu‚s ponerlo.
		 ...
		 if (header[0]==TRUCINDIV)
		 {
			...
			// baixa en la llista d'indicatius si hi ‚s.
				 ...
				 listIndics[posicio].indicatiu=tmp;
				 listIndics[posicio].canal=header[3]-1;
...
void gestioRxIntRadio(unsigned int origen,unsigned char *header,char *info)
{
	...
	//Si el mensaje viene del canal de trunking ha de hacer la
	//conversi¢n de indicativo.

	if (origen==4 && tempo>0)
	{

		for (n=0;n<MAX_TABLA && TablaPredefs[n]!=tempo;n++);
		if (n<MAX_TABLA)
			tempo=n+1;
		else
			tempo-=NumBase;
		...
		header[1]= (tempo&0xff00)>>8;
		header[2]= (tempo&0x00ff);

		if (header[3]!=CLEARED && header[3]!=SEND_PROG && header[3]!=ERR && header[3]!=EME
			&& header[3]!=SQLON && header[3]!=SQLOFF && header[3]!=TRUC_SOCI)
			{
				header[3]=7; //Si el origen es trunking, cualquier estado lo pasa a
								 //petici¢n de vez, excepto el SOS.
			}
	}
