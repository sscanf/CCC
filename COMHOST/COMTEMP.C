...
unsigned int FiRx=0,nBytes=0,ptrRx=0;
 ...
 do{
  while (!(kbhit()))
  {
	 ...
	 if (FiRx==1) //  Rx de CGC -> Tx a VAX
	 {
	  ...
	  printf("En fase de connexi¢ DMQ_CAMB_EST.\n");
...
void interrupt cojebuff1(__CPPARGS)
{
...
void TxMsg(void)
{
 ...
 for(;sts_value!=PAMS__SUCCESS && time<TIMEOUT;time++)
  sts_value= pams_put_msg(bufferRx,&priority,&target,&oclass,
...
void RxMsg(void)
{
  ...
  else
  {
	  ...
	  sts_value=-1;
	  for(;sts_value!=PAMS__SUCCESS && sts_value!=PAMS__TIMEOUT;)
			 sts_value=pams_get_msgw(ibuffer,&rcv_priority,
			  &source,&iclass,&itype,&buffer_length,
			  &size,&time_out,&sel_addr,&psb,0,0,0,0,0);
	  ...
	  if (sts_value==PAMS__SUCCESS)
	  {
		...
		printf("Desconnexi¢ DMQ_ASSIGNA per TIMEOUT.\n");
  }
}
